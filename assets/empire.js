...
var ProductDetails = function () {
  ...
  createClass(ProductDetails, [{
    ...
    value: function _initGallery() {
      ...
      this.gallery = new js$1(this.$slider[0], flickityOptions);
    ...
    value: function _bindGallery() {
      ...
      this.gallery.on('select', function () {
        _this2._updateActiveSlide(_this2.gallery.selectedIndex);

        for (var i = 0; i < _this2.$slides.length; i++) {
          var video = _this2.$slider.find(_this2.slides + ':nth-of-type(' + (i + 1) + ') [data-video]');

          if (video.length) {
            _this2.videoArray[i].onPauseClick();
          }
        }

        if (_this2.videoArray[_this2.gallery.selectedIndex] && _this2.settings.enable_video_autoplay) {
          _this2.videoArray[_this2.gallery.selectedIndex].autoplay();
        }
      });
      ...
      this.$paginationTrigger.on('click.gallery', function (event) {
        event.preventDefault();
        var $target = jquery$1(event.currentTarget);
        _this2.gallery.select($target.attr('data-product-thumbnail'), false, false);
      });
    }
...
function getData(el, name, options, sanitize) {
  ...
  if (!el.hasAttribute(attr)) return options[name] || defaults$1[name];
...
function parseItem(el) {
  ...
  return {
    ...
    templateRender: options.templateRender || defaults$1.templateRender,
 ...
 * @param {Settings} options
 */
function update$1$1(el, options) {
  if (!el) return;
...
//# sourceMappingURL=empire.js.map?1522961852416
